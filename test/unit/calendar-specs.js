...
describe('Calendar.js', () => {
  ...
  beforeEach(async () => {
    ...
    calendar = new Calendar(tccDir, bundleID);
  });
  ...
  describe('getDB()', () => {
    ...
    it('creates a new DB with a table named "access" if none existed in the first place', async () => {
      ...
      (await calendar.getCalendarRowCount()).should.equal(0);
    });
  ...
  describe('enableCalendarAccess()', () => {
    it('adds an item to the "access" table called "kTCCServiceCalendar"', async () => {
      await calendar.getCalendarRowCount().should.eventually.equal(0);
      await calendar.enableCalendarAccess();
      await calendar.getCalendarRowCount().should.eventually.equal(1);
    });
  ...
  describe('disableCalendarAccess()', () => {
    ...
    it('can enable and then disable', async () => {
      ...
      await calendar.enableCalendarAccess();
      await calendar.getCalendarRowCount().should.eventually.equal(1);
      await calendar.disableCalendarAccess();
      await calendar.getCalendarRowCount().should.eventually.equal(0);
    });

    it('does nothing if disableCalendarAccess called without calendar access being enabled', async () => {
      await calendar.disableCalendarAccess().should.not.be.rejected;
    });
