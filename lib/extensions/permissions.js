...
async function execSQLiteQuery (db, query, ...queryParams) {
  ...
  try {
    ...
    throw new Error(
      `Cannot execute SQLite query "${formattedQuery.join('')}" to '${db}'. ` +
 ...
 * `SERVICES` keys.
 * @param {string} sharedResourcesDir - the path to Simulator `data` root
 * @returns {string} - The current status: yes/no/unset
 * @throws {Error} If there was an error while retrieving permissions.
 */
async function getAccess (bundleId, serviceName, sharedResourcesDir) {
  const internalServiceName = toInternalServiceName(serviceName);
  const dbPath = path.resolve(sharedResourcesDir, 'Library', 'TCC', 'TCC.db');
  for (const [sqlValue, status] of [['0', STATUS_NO], ['1', STATUS_YES]]) {
    const count = await execSQLiteQuery(
      dbPath,
      `SELECT count(*) FROM 'access' WHERE client='?' AND allowed=? AND service='?'`,
      bundleId, sqlValue, internalServiceName
    );
