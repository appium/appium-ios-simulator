...
const VALID_COORDINATES_REGEX = /\{\s*\-?\d+\.?\d+?\s*,\s*\-?\d+\.?\d+?\s*\}/ig;
...
function validCoordinates (coordinates) {
  return VALID_COORDINATES_REGEX.test(coordinates);
}

export { killAllSimulators, endAllSimulatorDaemons, safeRimRaf, simExists, installSSLCert, uninstallSSLCert, execSQLiteQuery, validCoordinates };
