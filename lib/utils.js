...
async function killApp (appName, isForceKill = false) {
  let args = ['-x', appName];
  if (isForceKill) {
    args.splice(0, 0, '-9');
  }
  try {
    ...
    if (!_.isUndefined(e.code)) return e.code;
    log.error(`Got unexpected error while trying to kill ${appName}`);
    throw e;
  }
}
...
async function killAllSimulators (timeout = DEFAULT_SIM_SHUTDOWN_TIMEOUT) {
  ...
  try {
    await exec('xcrun', ['simctl', 'shutdown', 'booted'], {timeout: timeout});
    return;
  } catch (e) {
    log.warn(`Cannot shutdown ${appName} via simctl after ${timeout} ms (${e.message}). Using forced process kill...`);
  }

  const rc = await killApp(appName, true);
  if (rc > 1) {
    log.errorAndThrow(`Cannot forcefully terminate ${appName}. pkill error code: ${rc}`);
  }
...
export { killAllSimulators, endAllSimulatorDaemons, safeRimRaf, simExists, installSSLCert, uninstallSSLCert, execSQLiteQuery, enableCalendarAccess };
