...
async function forceKillAll (appName) {
  try {
    await exec('pkill', ['-9', '-x', appName]);
  } catch (e) {
    if (e.code === 1) {
      log.debug(`${appName} is not running. Continuing...`);
      return;
    }
    log.error(`pkill error ${e.code} while trying to forcefully kill ${appName}`);
    throw e;
  }
}
...
async function killAllSimulators (timeout = DEFAULT_SIM_SHUTDOWN_TIMEOUT) {
  ...
  try {
    ...
    log.error(`pgrep error ${e.code} while detecting whether ${appName} is running. Trying to kill anyway...`);
  }

  try {
    await exec('pkill', ['-x', appName]);
  } catch (e) {
    if (e.code === 1) {
      log.debug(`${appName} is not running. Continuing...`);
      return;
    }
    log.error(`pkill error ${e.code} while trying to gracefully kill ${appName}`);
  }
  ...
  try {
    ...
    if (remainingDevices.length !== 0) {
      log.warn(`The following devices are not in the correct state after ${timeout} ms:`);
      for (let device of remainingDevices) {
        log.warn(`    ${device}`);
      }
      log.warn(`Sending force kill signal to shutdown them all`);
      await forceKillAll(appName);
      return;
    }
    throw err;
  }
