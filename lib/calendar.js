...
async function getCalendarRowCount (db, bundleID) {
 ...
 */
async function enableCalendarAccess (udid, bundleID) {
...
/**
 * Disallow calendar access for UDID and bundleID
 * @param {string} udid 
 * @param {string} bundleID 
 */
async function disableCalendarAccess (udid, bundleID) {
  let pathToKeychain = path.resolve(new Simulator(udid).getDir());
  let db = await Calendar.getCalendarDB(pathToKeychain);
...
Calendar = {getCalendarDB, getCalendarRowCount, enableCalendarAccess, disableCalendarAccess};
