...
export default class Calendar {
  ...
  constructor (sharedResourcesDir, bundleID) {
  ...
  async getCalendarRowCount () {
    let db = await this.getDB();
    let count = await execSQLiteQuery(db, `SELECT count(*) FROM access WHERE client='?';`, this.bundleID);
    return parseInt(count.stdout.split('=')[1], 10);
  } 

  async  hasCalendarAccess () {
    let count = await execSQLiteQuery(await this.getDB(), `SELECT count(*) FROM access WHERE client='?' AND allowed=1;`, this.bundleID);
    return parseInt(count.stdout.split('=')[1], 10) > 0;
  }

  async enableCalendarAccess () {
    let db = await this.getDB();

    if ((await this.getCalendarRowCount(db, this.bundleID)) > 0) {
      ...
      await execSQLiteQuery(db, `INSERT INTO 'access' VALUES ('kTCCServiceCalendar', '?', 0, 1, 1, 0, 0);`, this.bundleID);
    }
  }

  async disableCalendarAccess () {
    let db = await this.getDB();

    if ((await this.getCalendarRowCount()) > 0) {
