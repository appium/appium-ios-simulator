...
async function getSimulator (xcodeVersion) {
  if (!xcodeVersion) {
  ...
  if (majorVersion < 6) {
    throw new Error(`Tried to use an iOS simulator with xcode ` +
                    `version ${this.xcodeVersion} but only Xcode version ` +
                    `6.0.0 and up are supported`);
  } else if (majorVersion === 6) {
    log.info(`Constructing iOS simulator for Xcode version 6.*.*`);
    return new SimulatorXcode6(xcodeVersion);
  } else {
    throw new Error(`Xcode version ${this.xcodeVersion} is ` +
                    `not yet supported`);
  }
}
