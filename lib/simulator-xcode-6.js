...
class SimulatorXcode6 {
  ...
  async cleanCustomApp (appFile, appBundleId) {
    log.info(`Cleaning app data files`);

    var appDirs = await this.getAppDirs(appFile, appBundleId);
    if (appDirs.length === 0) {
      log.info("Couldn't find app directories to delete. Probably it's not " +
               "installed");
      return;
    }
