...
import { fs, tempDir, mkdirp, plist, timing } from 'appium-support';
...
class SimulatorXcode6 extends EventEmitter {
  ...
  /**
   * Enable/Disable dark mode. This only works on Xcode 11+/iOS 13+.
   * This function can be called before the simulator has been run.
   *
   * @param {boolean} isEnabled - Whether or not to enable it (true by default)
   */
  async setDarkMode (/*isEnabled = true*/) { // eslint-disable-line require-await
    log.info(`Unable to set dark mode in Xcode version '${this.xcodeVersion}'`);
  }
  ...
  async ps () {
    ...
    */
    return servicesMatch[1]
      .split('\n')
      .map((line) => /^\s*(\d+)\s+[\d-]+\s+([\w.]+:)?([\w.]+)/.exec(line))
      .filter(Boolean)
      .map((match) => ({
