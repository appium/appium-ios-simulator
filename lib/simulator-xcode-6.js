...
class SimulatorXcode6 {
  ...
  setScaleFactor (newScaleFactor) {
    ...
    if (supportedScales.indexOf(newScaleFactor) < 0) {
      let msg = 'Only "' + supportedScales + '" values are supported as scale factors. "' +
                newScaleFactor + '" is passed instead.';
      log.debug(msg);
      throw new Error(msg);
    }
  ...
  async run (startupTimeout = STARTUP_TIMEOUT) {
    ...
    if (!!this.scaleFactor) {
      let stat = await this.stat();
      let additionalArgs = ['-SimulatorWindowLastScale-com.apple.CoreSimulator.SimDeviceType.' +
                            stat.name.replace(/\s+/g, '-'), this.scaleFactor];
  ...
  async tailLogsUntil (bootedIndicator, timeoutMs) {
    ...
    await retryInterval(200, 100, async () => {
