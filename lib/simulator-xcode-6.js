...
class SimulatorXcode6 {
  ...
  async updateSettings (plist, updates) {
    return settings.updateSettings(this, plist, updates);
  }
  ...
  async updateSafariSettings (updates) {
    ...
    settings.updateSettings(this, 'mobileSafari', updates);
  }

  async updateLocale (language, locale, calendarFormat) {
    await settings.updateLocale(this, language, locale, calendarFormat);
  }
  ...
  async cleanSafari (keepPrefs = true) {
    ...
    let safariLibraryDir = path.resolve(this.getAppDataDir('com.apple.mobilesafari'), 'Library');
    let filesToDelete = [
