...
class SimulatorXcode6 {
  ...
  async openUrl (url, bufferTime = null) {
    const SAFARI_BOOTED_INDICATOR = 'MobileSafari[';
    const SAFARI_STARTUP_TIMEOUT = 15 * 1000;
    const EXTRA_STARTUP_TIME = 3 * 1000;

    // extra time added before opening a url by default.
    // override by setting to zero
    if (bufferTime === null) {
      let version = await this.getPlatformVersion();
      ...
      if (version === 8) {
        ...
        log.warn(`No bufferTime set for opening url on ios version ${version}`);
        log.warn('defaulting to 1 second');
        bufferTime = 1 * 1000;
      }
    }

    if (await this.isRunning()) {
      await B.delay(bufferTime);
  ...
  async tailLogsUntil (bootedIndicator, timeoutMs) {
    ...
    await retryInterval(200, 2000, async () => {
