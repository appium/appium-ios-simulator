...
class SimulatorXcode6 {
  ...
  async run () {
    ...
    const BOOTING_TIMOUT = 20000;
    const BOOTED_POLLING_INTERVAL = 200;

    // start simulator
    let simulatorApp = path.resolve(await getXcodePath(), 'Applications', 'Simulator.app');
    ...
    // wait for the simulator to be 'booted'
    await retryInterval(parseInt(BOOTING_TIMOUT/BOOTED_POLLING_INTERVAL, 10),
                        BOOTED_POLLING_INTERVAL, async () => {
      let stat = await this.stat();
      if (stat.state !== 'Booted') {
        throw new Error("State wasn't booted");
      }
    });
  }
