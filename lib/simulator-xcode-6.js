...
class SimulatorXcode6 extends EventEmitter {
  ...
  async backupKeychains () {
    ...
    const cmdLine = ['-r', backupPath, `${this.keychainPath}${path.sep}*`];
    log.debug(`Creating keychains backup as '${path.basename(backupPath)}'` +
              `with 'zip ${cmdLine.join(' ')}' command line`);
    await exec('zip', cmdLine);
   ...
   * of file patterns to exlude from restore. The format
  ...
  async restoreKeychains (excludePatterns = []) {
    ...
    const cmdLine = ['-o', this._keychainsBackupPath,
                     ...(_.flatMap(excludePatterns.map((x) => ['-x', x]))),
                     '-d', this.keychainPath];
    log.debug(`Restoring keychains from '${path.basename(this._keychainsBackupPath)}' ` +
              `with 'unzip ${cmdLine.join(' ')}' command line`);
    await exec('unzip', cmdLine);
