...
class SimulatorXcode6 extends EventEmitter {
  ...
  async backupKeychains () {
    ...
    if (await fs.exists(this._keychainsBackupPath)) {
      log.debug(`Deleting the previosly stored backup at '${this._keychainsBackupPath}'`);
    ...
    log.debug(`Creating keychains backup with 'zip ${cmdLine.join(' ')}' command`);
  ...
  async restoreKeychains (excludePatterns = []) {
    ...
    const cmdLine = ['-o', this._keychainsBackupPath,
                     ...
                     '-d', '/'];
    log.debug(`Restoring keychains with 'unzip ${cmdLine.join(' ')}' command`);
