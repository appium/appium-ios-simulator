...
import tail from 'tailing-stream';
...
class SimulatorXcode6 {
  ...
  async getBootedIndicatorString () {
    ...
    switch (await this.getPlatformVersion()) {
      ...
      case '8.2':
      case '8.3':
        indicator = 'MC: Managed Configuration completed migration';
        break;
      case '8.4':
  ...
  async run () {
    ...
    await exec('open', args, {timeout: STARTUP_TIMEOUT});
    ...
    await retryInterval(30, 200, async () => {
      if (!await fs.exists(simLog)) {
        throw new Error('not ready yet');
      }
    });
    let stream = tail.createReadStream(simLog, {timeout: 10*1000});

    let booted = new B((resolve, reject) => {
      stream.on('data', (line) => {
        if (line.toString().indexOf(bootedIndicator) > -1) {
          resolve();
        }
      });
      stream.on('error', (e) => {
      ...
      stream.on('end', () => {
        reject(new Error(`sim log ended and never encountered string which indicates device booted: ${bootedIndicator}`));
      });
    });
