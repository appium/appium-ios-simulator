...
class SimulatorXcode6 extends EventEmitter {
   ...
   */
  async reduceMotion (bundleID, reduceMotion = true) {
    if (await this.isRunning()) {
      await this.endSimulatorDaemon();
    } else if (await this.isFresh()) {
      await this.launchAndQuit(false, STARTUP_TIMEOUT);
    }
    await settings.reduceMotion(this, bundleID, reduceMotion);
  }

  async readSettings (plist) {
    return await settings.readSettings(this, plist);
  }
