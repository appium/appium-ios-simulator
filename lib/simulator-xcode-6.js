...
class SimulatorXcode6 extends EventEmitter {
  ...
  constructor (udid, xcodeVersion) {
    ...
    this.permissions = new Permissions(xcodeVersion, this.getDir());
  }
   ...
   * @param {string} status - one of available statuses
   * @throws {Error} If there was an error while changing permissions
   */
  async setPermission (bundleId, serviceName, status) {
    await this.permissions.setAccess(serviceName, bundleId, status);
    // Wait a bit to propagate the change and restart springboard in order to apply it
    await B.delay(100);
    await simctl.spawn(this.udid, ['launchctl', 'stop', SPRINGBOARD_BUNDLE_ID]);
    log.debug(`Set ${serviceName} access for '${bundleId}' to '${status}'`);
  }
  ...
  async getPermission (bundleId, serviceName) {
    ...
    log.debug(`Got ${serviceName} access for '${bundleId}': ${result}`);
    return result;
  }
