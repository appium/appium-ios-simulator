...
class Certificate {
  ...
  async add (dir) {
    if (!this.data)
      this.data = await Certificate.pemFileToDer(this.pemFilename);
    if (!this.subject)
      this.subject = await Certificate.getSubject(this.pemFilename);
    if (!this.fingerprint)
      this.fingerprint = await Certificate.getFingerPrint(this.data);

    let trustStore = new TrustStore(dir);
    return trustStore.addRecord(this.fingerprint, tset, this.subject, this.data);
  }
  ...
  async has (dir) {
    ...
    let trustStore = new TrustStore(dir);
    let records = await trustStore.getRecords(this.subject);
