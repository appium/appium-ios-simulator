...
class TrustStore {
  ...
  async addRecord (sha1, tset, subj, data) {
    if (await this.hasRecords(subj)) {
      return await execSQLiteQuery(await this.getDB(), `UPDATE tsettings SET sha1=x'?', tset='?', data=x'?' WHERE subj='?'`, sha1, tset, data, subj);
    } else {
      return await execSQLiteQuery(await this.getDB(), `INSERT INTO tsettings (sha1, subj, tset, data) VALUES (x'?', '?', '?', x'?')`, sha1, subj, tset, data);
    }
  }
