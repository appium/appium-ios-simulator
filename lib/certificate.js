...
class Certificate {
  ...
  async getSubject () {
    ...
    return subject.toString().match(subRegex)[1];
  }
...
class TrustStore {
  ...
  async addRecord (sha1, tset, subj, data) {
    ...
    if (existingRecords.length > 0) {
      return this.db.runAsync(`UPDATE tsettings SET sha1=?, tset=?, data=? WHERE subj=?`, [sha1, tset, data, subj]);
    } else {
      return this.db.runAsync(`INSERT INTO tsettings (sha1, subj, tset, data) VALUES (?, ?, ?, ?)`, [sha1, subj, tset, data]);
    }
  }
