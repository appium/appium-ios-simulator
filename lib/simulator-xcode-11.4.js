...
class SimulatorXcode11_4 extends SimulatorXcode11 {
  ...
  async addCertificate (payload, opts = {}) {
    ...
    try {
      const certPath = path.resolve(tmpRoot, 'certificate.pem');
      log.info(`Installing SSL ${isRoot ? 'root ' : ''}certificate '${_.truncate(payload, {length: 20})}'`);
      if (isRoot) {
        await this.simctl.addRootCertificate(certPath);
      } else {
        await this.simctl.addCertificate(certPath);
      }
    } finally {
  ...
  async setPermissions (bundleId, permissionsMapping) {
    log.debug(`Setting ${JSON.stringify(permissionsMapping, null, 2)} access for '${bundleId}'`);
    ...
    if (!_.isEmpty(nonNativePerms)) {
      log.info(`The permissions ${JSON.stringify(permissionsMapping, null, 2)} ` +
        `have not been recognized as native`);
