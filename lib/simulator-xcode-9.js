...
class SimulatorXcode9 extends SimulatorXcode8 {
   ...
   *   device
   *   - {string} simulatorWindowCenter: string formatted as "{x,y}" coordinates used
   *   to place the Simulator window on startup
   */
  async run (opts = {}) {
    opts = Object.assign({
      devicePreferences: {},
    ...
    if (opts.simulatorWindowCenter) {
      if (validCoordinates(opts.simulatorWindowCenter)) {
        opts.devicePreferences.SimulatorWindowCenter = opts.simulatorWindowCenter;
      } else {
        log.warn(`Invalid simulatorWindowCenter value: ${opts.simulatorWindowCenter}. Valid string format "{x,y}"`);
      }
    }
