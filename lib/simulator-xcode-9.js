...
class SimulatorXcode9 extends SimulatorXcode8 {
   ...
   *   device
   *   - {object} simulatorWindowCenter: object with x,y coordinates to place the Simulator window
   *   on startup
   */
  async run (opts = {}) {
    opts = Object.assign({
    ...
    if (opts.simulatorWindowCenter) {
      if (!isNaN(opts.simulatorWindowCenter.x) && !isNaN(opts.simulatorWindowCenter.y)) {
        let x = parseFloat(opts.simulatorWindowCenter.x), y = parseFloat(opts.simulatorWindowCenter.y);
        opts.devicePreferences.SimulatorWindowCenter = `{${x},${y}}`;
      } else {
        log.warn(`Invalid simWindowCenter value: ${opts.simulatorWindowCenter}. Valid format {x:x, y:y}`);
      }
    }
