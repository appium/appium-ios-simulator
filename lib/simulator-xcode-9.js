...
class SimulatorXcode9 extends SimulatorXcode8 {
  ...
  async configureLocalization (opts = {}) {
    ...
    log.info('==============1');
    log.info(previousAppleLanguages);
    log.info(globalPrefs.AppleLanguages);

    log.info('==============2');
    log.info(JSON.stringify(previousAppleLanguages));
    log.info(JSON.stringify(globalPrefs.AppleLanguages));

    if (globalPrefs.AppleLanguages) {
      if (previousAppleLanguages && _.isEqual(previousAppleLanguages, globalPrefs.AppleLanguages)) {
        log.info(
