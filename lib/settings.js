...
const PLIST_PATH_IDENTIFIER = Object.freeze({
...
/**
 * Updates plist files with the given PLIST_PATH_IDENTIFIER
 *
 * @param {object} sim Simulator instance
 * @param {string} plist One of PLIST_PATH_IDENTIFIER constants
...
async function updateLocationSettings (sim, bundleId, authorized) {
  ...
  const updated = await updateSettings(sim, PLIST_PATH_IDENTIFIER.LOCATION_CACHE, {
  ...
  const paths = await plistPaths(sim, PLIST_PATH_IDENTIFIER.LOCATION_CLIENTS);
  if (_.isEmpty(paths)) {
    return false;
  }
...
async function setReduceMotion (sim, reduceMotion = true) {
  ...
  await updateSettings(sim, PLIST_PATH_IDENTIFIER.ACCESSIBLITY_SETTINGS, {
...
async function updateSafariGlobalSettings (sim, settingSet) {
  ...
  const promises = [];
  for (const [file, safariSettingSet] of _.toPairs(await readSettings(sim, PLIST_PATH_IDENTIFIER.GLOBAL_MOBILE_SAFARI))) {
    const newSettings = {};
...
async function updateSafariUserSettings (sim, settingSet) {
  ...
  const promises = [];
  for (const [file, userSettingSet] of _.toPairs(await readSettings(sim, PLIST_PATH_IDENTIFIER.USER_SETTINGS))) {
...
async function updateLocale (sim, language, locale, calendarFormat) {
  const globalPrefs = (await plistPaths(sim, PLIST_PATH_IDENTIFIER.GLOBAL_PREFERENCES))[0];
  if (!globalPrefs) {
...
export {
  ...
  readSettings, PLIST_PATH_IDENTIFIER,
};
