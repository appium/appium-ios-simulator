...
async function setDarkMode (sim, darkMode = true) {
  log.debug(`Setting dark mode ${darkMode ? 'on' : 'off'}`);
  const paths = await plistPaths(sim, 'uiStyleSettings');
  for (const file of paths) {
    await update(file, {
      UserInterfaceStyleMode: darkMode ? 2 : 1,
    });
  }
}
...
export {
  update, updateSettings, updateLocationSettings, setReduceMotion, setDarkMode,
