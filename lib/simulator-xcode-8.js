...
const STARTUP_TIMEOUT = 240 * 1000;
    ...
    try {
      ...
      await waitForCondition(async () => {
        try {
          ...
          log.debug(`Waiting for simulator to boot: ${err.message}`);
          lastError = err.stderr || err.message;
          if ((err + '').includes('Unable to lookup in current state: Shutdown') && _.isFunction(bootFn)) {
            await bootFn();
          }
        }
        return false;
