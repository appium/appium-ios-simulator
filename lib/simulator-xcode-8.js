    ...
    try {
      await waitForCondition(async () => {
        try {
          ...
          const {stdout} = await this.simctl.exec('launch', {
            args: [this.udid, MOBILE_SAFARI_BUNDLE_ID]
          });
      ...
      log.errorAndThrow(`Safari cannot open '${url}' after ${timer.getDuration().asSeconds.toFixed(3)}s ` +
