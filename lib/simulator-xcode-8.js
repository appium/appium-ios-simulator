...
import * as fbsimctl from './fbsimctl-utils';
...
class SimulatorXcode8 extends SimulatorXcode7 {
  ...
  async activateWindow () {
    try {
      ...
      log.warn(`Cannot focus Simulator window with fbsimctl. Defaulting to AppleScript. ` +
        `Original error: ${err.message}`);
      return await super.activateWindow();
    }
  }
  ...
  async setGeolocation (latitude, longitude) {
    try {
      ...
      log.warn(`Cannot set geolocation with fbsimctl. Defaulting to AppleScript. Original error: ${e.message}`);
      const output = await this.executeUIClientScript(`
        tell application "System Events"
          tell process "Simulator"
